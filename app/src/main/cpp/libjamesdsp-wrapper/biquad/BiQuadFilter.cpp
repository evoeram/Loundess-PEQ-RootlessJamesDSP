/*
 * Ported from EqualizerAPO's filters/BiQuadFilter.cpp.
 * Original: Copyright (C) 2014 Jonas Thedering, GPLv2.
 *
 * Changes for the RootlessJamesDSP / Android NDK port:
 *  - Removed stdafx.h.
 *  - Replaced std::wstring channel names with std::string.
 *  - Added an ARM64 NEON float64x2 (2-wide) processing path, mirroring the
 *    SSE2 __m128d path. ARM32 and other targets fall back to scalar.
 *  - Kept the x86 AVX512/AVX2/SSE2 paths, guarded so they only compile on x86.
 *  - FTZ/DAZ (denormal flush): kept the MXCSR path on x86; on AArch64 the FPCR
 *    FZ bit is toggled via inline asm. On targets without either, the filter
 *    relies on the -Ofast/-ffast-math flags already set in CMakeLists.txt.
 */
...
std::vector<std::string> BiQuadFilter::initialize(float sampleRate, unsigned maxFrameCount, std::vector<std::string> channelNames)
{
    ...
    if (isCornerFreq && (type == BiQuad::LOW_SHELF || type == BiQuad::HIGH_SHELF))
    {
        ...
        double centerFreqFactor = pow(10.0, std::abs(dbGain) / 80.0 / s);
...
namespace
{
    // RAII helper to enable flush-to-zero / denormals-are-zero on the audio
    // thread, restored on scope exit. Faithful to the original MXCSR logic on
    // x86; AArch64 toggles the FPCR FZ bit.
    struct DenormalFlushGuard
    {
#if (defined(__x86_64__) || defined(__i386__) || defined(_M_X64) || defined(_M_IX86)) && !defined(__ARM_NEON)
        unsigned old_mxcsr;
        DenormalFlushGuard() : old_mxcsr(_mm_getcsr())
        {
            // FTZ (bit 15 = 0x8000) | DAZ (bit 6 = 0x0040)
            _mm_setcsr(old_mxcsr | 0x8040);
        }
        ~DenormalFlushGuard() { _mm_setcsr(old_mxcsr); }
#elif defined(__aarch64__) && !defined(_M_ARM64)
        unsigned long old_fpcr;
        DenormalFlushGuard()
        {
            __asm__ __volatile__("mrs %0, fpcr" : "=r"(old_fpcr));
            // FZ is bit 24 of FPCR
            __asm__ __volatile__("msr fpcr, %0" :: "r"(old_fpcr | (1UL << 24)));
        }
        ~DenormalFlushGuard()
        {
...
void BiQuadFilter::process(double** output, double** input, unsigned frameCount)
{
    ...
    const unsigned neon2_width = 2;
    unsigned num_neon2_chunks = ((unsigned)channelCount - processedChannels) / neon2_width;
    if (num_neon2_chunks > 0) {
        process_neon2(output, input, frameCount, processedChannels, num_neon2_chunks * neon2_width);
        processedChannels += num_neon2_chunks * neon2_width;
    }
#endif
...
#if defined(BIQUAD_HAS_NEON_F64)
// ARM64 NEON optimized processing for 2 channels (stereo) at a time.
// AArch64 NEON provides float64x2_t (2-wide double) with fused multiply-add.
void BiQuadFilter::process_neon2(double** output, double** input, unsigned frameCount, unsigned startChannel, unsigned numChannels)
{
    const unsigned simd_width = 2;
    for (unsigned i = startChannel; i < startChannel + numChannels; i += simd_width)
    {
        const float64x2_t _a0 = vld1q_f64(&a0[i]);
        const float64x2_t _b1 = vld1q_f64(&b1[i]);
        const float64x2_t _b2 = vld1q_f64(&b2[i]);
        const float64x2_t _a1 = vld1q_f64(&a1[i]);
        const float64x2_t _a2 = vld1q_f64(&a2[i]);

        float64x2_t _x1 = vld1q_f64(&x1[i]);
        float64x2_t _x2 = vld1q_f64(&x2[i]);
        float64x2_t _y1 = vld1q_f64(&y1[i]);
        float64x2_t _y2 = vld1q_f64(&y2[i]);

        for (unsigned j = 0; j < frameCount; ++j)
        {
            float64x2_t _sample = vdupq_n_f64(0.0);
            _sample = vsetq_lane_f64(input[i + 0][j], _sample, 0);
            _sample = vsetq_lane_f64(input[i + 1][j], _sample, 1);

            float64x2_t result = vmulq_f64(_a0, _sample);
            result = vfmaq_f64(result, _b1, _x1); // result + b1*x1
            result = vfmaq_f64(result, _b2, _x2); // result + b2*x2
            result = vfmsq_f64(result, _a1, _y1); // result - a1*y1
            result = vfmsq_f64(result, _a2, _y2); // result - a2*y2

            _x2 = _x1; _x1 = _sample;
            _y2 = _y1; _y1 = result;

            double result_array[2];
            vst1q_f64(result_array, result);
            output[i + 0][j] = result_array[0];
            output[i + 1][j] = result_array[1];
        }
        vst1q_f64(&x1[i], _x1);
...
{
    for (size_t i = startChannel; i < channelCount; ++i)
    {
